...
int orientation(point p, point q, point r) {
  ...
  return (val > 0) ? 1 : 2; // 1 for cw or 2 for ccw
}

bool areOnSeg(point p, point q, point r) // Checks if r is on segment pq.
{
...
bool doesIntersect(point p1, point q1, point p2, point q2) {
  ...
  if (o1 != o2 && o3 != o4)
    return true;
  // Check co-linearity
  if (o1 == 0 && areOnSeg(p1, p2, q1)) return true;
  if (o2 == 0 && areOnSeg(p1, q2, q1)) return true;
  if (o3 == 0 && areOnSeg(p2, p1, q2)) return true;
  if (o4 == 0 && areOnSeg(p2, q1, q2)) return true;
...
bool isTriangle(segment seg1, segment seg2, segment seg3) {
  bool seg1_2 = doesIntersect(point(seg1.x1, seg1.y1), point(seg1.x2, seg1.y2), point(seg2.x1, seg2.y1), point(seg2.x2, seg2.y2));
  bool seg2_3 = doesIntersect(point(seg3.x1, seg3.y1), point(seg3.x2, seg3.y2), point(seg2.x1, seg2.y1), point(seg2.x2, seg2.y2));
  bool seg1_3 = doesIntersect(point(seg1.x1, seg1.y1), point(seg1.x2, seg1.y2), point(seg3.x1, seg3.y1), point(seg3.x2, seg3.y2));

  return seg1_2 and seg2_3 and seg1_3;
}
...
int main() {
  ...
  while (cin >> n) {
    ...
    ull count_triangles = 0;
    for (int j = 0; j < n; ++j) {
      for (int i = j+1; i < n; ++i) {
        for (int k = i+1; k < n; ++k) {
//          cout << "Checking " << j << " " << i << " " << k << endl;
