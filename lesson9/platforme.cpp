...
ull calc_length(ull x, vector<pair<point, point>> & platforms){
  pair<point, point>& plat = platforms[x];
  auto start = plat.first;
  auto end = plat.second;
  ull min_start = start.y;
  for (int i = 0; i < platforms.size(); ++i) {
    if (i == x){
      continue;
    }
    auto current = platforms[i];
    auto current_start = current.first;
    auto current_end = current.second;

    if (current_start.y > start.y){
      continue;
    }

    if (current_start.x <= start.x and current_end.x >= start.x + 1){
      if (start.y > current_start.y){
        if (start.y - current_start.y < min_start){
          min_start = start.y - current_start.y;
        }
      }
    }
  }
  ...
  for (int i = 0; i < platforms.size(); ++i) {
    ...
    if (current_start.y > end.y){
      continue;
    }

    if (current_start.x <= end.x - 1 and current_end.x >= end.x){
      if (end.y > current_start.y){
        if (end.y - current_start.y < min_end){
          min_end = end.y - current_start.y;
        }
      }
    }
  }
...
int main() {
  ...
  ull N;
  while (cin >> N){
    vector<pair<point, point>> platforms;
    ull x1, x2, y;
    for (int i = 0; i < N; ++i) {
      cin >> y >> x1 >> x2;
    ...
    ull length = 0;
    for (int j = 0; j < N; ++j) {
      length += calc_length(j, platforms);
    }
    cout << length << endl;
  }
