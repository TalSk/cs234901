...
double dist(point a, point b, point c){ // Returns distance of a from bc
  point x;
  return distToLineSegment(a, b, c, x) / 2;
}
int main() {
  ...
  double x, y;
  while (cin >> tests){
    for (int aaaa = 0; aaaa < tests; ++aaaa) {
      cin >> ni;
      ...
      for (ull i0 = 0, i1 = 1; i0 < inner.size(); ++i0, i1 = (i0 + 1) % inner.size())
        for (ull j0 = 0, j1 = 1; j0 < outer.size(); ++j0, j1 = (j0 + 1) % outer.size()) {
          minimum_distance = min(minimum_distance, dist(inner[i0], outer[j0], outer[j1]));
          minimum_distance = min(minimum_distance, dist(outer[j0], inner[i0], inner[i1]));
        }
      printf("%.8f\n", minimum_distance);
    }
