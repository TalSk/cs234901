...
int main() {
  ...
  while(cin >> T){
    for (int Bla = 0; Bla < T; ++Bla) {
      ...
      ull m;
      cin >> m;
      vector<ull> medical_facilities(m);
      for (int j = 0; j < m; ++j) {
        cin >> medical_facilities[j];
        medical_facilities[j]--;
      }
      ull r;
      cin >> r;
      vector<pair<ii, ii>> edges(r);
      ull a, b, p ,t;
      for (int k = 0; k < r; ++k) {
        cin >> a >> b >> p >> t;
        a--;
      ...
      ull num_vertex = n * (s + 1) + 2;
      ull source_vertx = n * (s + 1);
      ull target_vertex = n * (s + 1) + 1;

      vector<iii> flow_edges;

      for (int l = 0; l < n; ++l) {
        for (int j = 0; j < s; ++j) {
          flow_edges.push_back({INF, {l + j * n, l + (j+1) * n}});
        }
      }

      flow_edges.push_back({g, {source_vertx, i}});
      for(auto & facility : medical_facilities) {
        for (int j = 0; j <= s; ++j) {
          flow_edges.push_back({INF, {facility + j * n, target_vertex}});
        }
      }

      ull a_p, b_p, p_p, t_p;
      for(auto & edge : edges){
        a_p = edge.first.first;
        b_p = edge.first.second;
        p_p = edge.second.first;
        t_p = edge.second.second;
        for (int j = 0; j < s; ++j) {
          if (j + t_p > s){
            break;
          }
          flow_edges.push_back({p_p, {a_p + j * n, b_p + (j+t_p) * n}});
        }
      }
      vector<map<int, int>> res(num_vertex);
      auto min_cut = EdmondsKarp(num_vertex, flow_edges, source_vertx, target_vertex, res);
      auto ans = min(g, min_cut);
      cout << ans << endl;
    }
  }
  return 0;
