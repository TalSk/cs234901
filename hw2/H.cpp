...
void dfs(const vvu &g, int s, vector<ull>& parents) {
  stack<int> q;
  q.push(s);
...
int main() {
  ...
  while (cin >> n){
    ...
    vector<iii> edges_1 = edges;
    vector<ull> parents(n);
    auto mst = Kruskal(edges_1, n);
    ...
    for (int j = 0; j < edges.size(); ++j) {
      auto x = edges[j].second.first;
      auto y = edges[j].second.second;
      auto it_x = find(path.begin(), path.end(), x);
      auto it_y = find(path.begin(), path.end(), y);
      auto found_x = it_x != path.end();
      auto found_y = it_y != path.end();
      if (found_x != found_y){
        edges_to_remove.insert(j);
      }
      if (found_x and found_y) {
        auto dist_x = distance(path.begin(), it_x);
        auto dist_y = distance(path.begin(), it_y);
        if (abs(dist_x - dist_y) != 1){
          edges_to_remove.insert(j);
        }
      }
    }
