...
// time: O(ElogV).
ull Kruskal(vector<iii>& edges, ull n, vector<ull> combined) {
  sort(edges.begin(), edges.end());
  ull mst_cost = 0;
  unionfind components(n);
  if (combined.size() >= 2){
    auto abcabc = combined[0];
    for (auto & vertexex : combined){
      if (vertexex != abcabc){
        components.unite(abcabc, vertexex);
      }
    }
  }
...
int main(){
  ...
  while(cin >> T){
    for (int bla = 0; bla < T; ++bla) {
      ...
      for (int i = 0; i < S; ++i) {
        cin >> temp;
        combined.push_back(temp - 1);
      }
      ull i, j, eij;
      vector<iii> edges;
      for (int k = 0; k < M; ++k) {
        cin >> i >> j >> eij;
        edges.push_back({eij + L, {i - 1, j - 1}});
      }
      auto cost = Kruskal(edges, N, combined);
      cout << cost << endl;
    }
  }
