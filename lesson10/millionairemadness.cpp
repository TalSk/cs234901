...
// time: O(ElogV).
ull Kruskal(vector<iii> &edges, ull n) {
...
// Gets the shortest path length from 0,0 to n-1, n-1. In this exercise, shortest
// means the one with the minimal maximum edge cost. (Therefore we relax with max)
ll dijkstra(vector<vector<ll>> & costs, ull M, ull N){
  vector<vector<ll>> dists(M, vector<ll>(N, INF));
  dists[0][0] = 0; // s = 0
  ...
  while(!q.empty()) {
    iii front = q.top();
    q.pop();
    ull i = front.second.first;
    ull j = front.second.second;
    ll cur_dist = front.first;
    if (cur_dist > dists[i][j])
      continue;
    // We are currently at vertex i,j. Try to walk up, down, left and right and
    // relax according to a maximum rule.
    if (i > 0) { // Up
      ll cost = costs[i - 1][j] - costs[i][j];
      if (cost < 0) cost = 0;
      cost = max(cur_dist, cost);
      if (cost < dists[i - 1][j]){
        // We found a lighter path to i-1,j
        dists[i - 1][j] = cost;
        q.push({cost, {i - 1, j}});
      }
    }
    ...
    if (j > 0) { // Left
      ll cost = costs[i][j - 1] - costs[i][j];
      if (cost < 0) cost = 0;
      cost = max(cur_dist, cost);
      if (cost < dists[i][j - 1]){
        // We found a lighter path to i,j-1
        dists[i][j - 1] = cost;
        q.push({cost, {i, j - 1}});
      }
    }
...
int main() {
  ...
  while (cin >> M) {
    cin >> N;
    vector<vector<ll>> costs(M, vector<ll>(N));
    ull tmp;
    if (M == 1 and N == 1) {
      cin >> tmp;
      cout << 0 << endl;
