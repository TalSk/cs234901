...
int main() {
  ...
  while(cin >> c >> r >> o){
    ...
    auto ch_robbers = robbers;
    if (robbers.size() >= 2) {
      ch_robbers = convex_hull(robbers);
    }
    ...
    for (int j = 0; j < o; ++j) {
      ...
      cin >> x >> y;
      point ord(x, y);
      if (PointInPolygon(ch_cops, ord)){
        status = SAFE;
      }
      else if (PointOnPolygon(ch_cops, ord) and cops.size() >= 3){
        status = SAFE;
      }
      else if (PointInPolygon(ch_robbers, ord)){
        status = ROBBED;
      }
      else if (PointOnPolygon(ch_robbers, ord) and robbers.size() >= 3){
        status = ROBBED;
      }
      switch (status) {
      case SAFE:
        cout << "     Citizen at (" << x << "," << y << ") is safe." << endl;
        break;
      case ROBBED:
        cout << "     Citizen at (" << x << "," << y << ") is robbed." << endl;
        break;
      default:
        cout << "     Citizen at (" << x << "," << y << ") is neither." << endl;
        break;
      }
    }
